    ...
    if (req.url.indexOf("singular") >= 0) {
      try {
        ...
        if (Array.isArray(json)) {
          if (json.length === 1) {
            return send.call(this, JSON.stringify(json[0]));
          } else if (json.length === 0) {
            return send(JSON.stringify({ error: "NOT FOUND" })).status(404);
          }
        }
      } catch (e) {
 ...
 * @param {import('express').Router} server
 *
 * @returns {import('express').Router}
 */
function setupAPI(router, server) {
